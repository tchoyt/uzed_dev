...
function cp_rootfs()
{
	...
	then
		tar xfvp ${ROOTFS_TAR_DIR}/*-*-*-armhf-*/armhf-rootfs-*.tar -C ${ROOTFS_INSTALL_DIR}
	else
...
function cp_modules()
{
	cp -rv ${INSTALL_MOD_PATH}/lib ${ROOTFS_INSTALL_DIR}/.
}

# Copy SoCFPGA applications
function cp_apps()
{
	cp -v ${SOCFPGA_SW_DIR}/axi_gp0_rw ${ROOTFS_INSTALL_DIR}/usr/local/bin/.
}
...
{
	sed -i '/127.0.1.1/c\127.0.1.1\t'"${BOARD_HOSTNAME}"'.localdomain\t'"${BOARD_HOSTNAME}" ${ROOTFS_INSTALL_DIR}/etc/hosts
	sed -i '/arm/c\'"${BOARD_HOSTNAME}" ${ROOTFS_INSTALL_DIR}/etc/hostname
}

# Write /etc/ssh/sshd_config
function wr_ssh_config()
{
	sed -i '/PermitRootLogin prohibit-password/c\PermitRootLogin yes' ${ROOTFS_INSTALL_DIR}/etc/ssh/sshd_config
}
